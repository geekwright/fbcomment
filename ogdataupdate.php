...
if($ogurl) $url=urldecode($ogurl);
if($ogimage) $ogimage=urldecode($ogimage);
if($ogtitle) $ogtitle=urldecode($ogtitle);
if($ogdescription) $ogdescription=urldecode($ogdescription);
...
if ($ogtitle) { update_fbc_og_meta ($url,'title',$ogtitle); exit(0); }
if ($ogdescription)  { update_fbc_og_meta ($url,'description',$ogdescription); exit(0); }
...
error_reporting(-1);
	...
	$sql = "SELECT image FROM ".$xoopsDB->prefix('fbc_og_meta').
	    " where url='{$q_url}' ";
	...
	$fn = mysql_real_escape_string($justfn);
...
global $xoopsDB;
	...
	$q_url=mysql_real_escape_string($url);
	$q_col=mysql_real_escape_string($col);
	$q_val=mysql_real_escape_string($val);

	$sql = "UPDATE ".$xoopsDB->prefix('fbc_og_meta').
	    " set {$q_col} = '{$q_val}', lastupdate = UNIX_TIMESTAMP() " .
	    " where url='{$q_url}' ";
	
	$result = $xoopsDB->queryF($sql);
	if ($result && !$xoopsDB->getAffectedRows()) { // database is OK but nothing to update
		$sql = "insert into ".$xoopsDB->prefix('fbc_og_meta').
		    " (url, {$col}, lastupdate) " .
		    " values('{$q_url}','{$val}', UNIX_TIMESTAMP() )";
		$result = $xoopsDB->queryF($sql);
	}
}
...
$url=mysql_real_escape_string($ogurl);

$ogtitle='';
if (!empty($_POST['fbc_dd_ogtitle'])) $ogtitle = urldecode($_POST['fbc_dd_ogtitle']);
$ogtitle = mysql_real_escape_string($ogtitle);
update_fbc_og_meta ($url,'title',$ogtitle);

$ogdescription='';
if (!empty($_POST['fbc_dd_ogdesc'])) $ogdescription = urldecode($_POST['fbc_dd_ogdesc']);
$ogdescription = mysql_real_escape_string($ogdescription);
...
error_reporting(-1);
	...
	if($error) redirect_header($ogurl, 5 ,_MD_FBCOM_IMAGE_UPLOAD_ERROR);
	else redirect_header($ogurl, 5 ,_MD_FBCOM_UPDATE_OK);
}
// chmod("/somedir/somefile", 0644);

/*-----------�q�X���G��--------------*/
include XOOPS_ROOT_PATH."/header.php";
$xoopsOption['template_main'] = "fbcomment_index.html";
