...
if (!empty($b_fbcomment_caller_name)) {
    ...
    if (file_exists($b_fbcomment_caller_name)) {
        require_once $b_fbcomment_caller_name;
    }
}
...
function b_fbcomment_addfiledropform($metas, &$block)
{
    ...
    $block['formddscript'] = XOOPS_URL . '/modules/fbcomment/assets/js/filedrag.js';
    ...
    if (empty($GLOBALS['b_fbccom_addfiledropform_called'])) {
        ...
        // add our styles
        $xoTheme->addStylesheet(XOOPS_URL . '/modules/fbcomment/assets/css/module.css');
...
function b_fbcomment_core_show($options)
{
    ...
    switch ($options[0]) {
        ...
        default: // AUTO = 0 FORCE = 3
            ...
            $port = empty($_SERVER['SERVER_PORT']) ? $defaultport : (int)$_SERVER['SERVER_PORT'];
    ...
    $appid  = $moduleHelper->getConfig('facebook-appid');
    $admins = $moduleHelper->getConfig('facebook-admins');
    ...
    $useSmartyVar = $moduleHelper->getConfig('use-smarty-for-ogdata');
    ...
    */
    ...
    $image       = $moduleHelper->getConfig('default-og-image');
    ...
    if ($result) {
        if ($myrow = $xoopsDB->fetchArray($result)) {
            if (!empty($myrow['image'])) {
                $metas['og:image'] = XOOPS_URL . '/uploads/fbcomment/' . $myrow['image'];
            }
            if (!empty($myrow['title'])) {
                $metas['og:title'] = $myrow['title'];
            }
            if (!empty($myrow['description'])) {
                $metas['og:description'] = $myrow['description'];
            }
        }
    }
    ...
    foreach ($metas as $property => $content) {
        if ($useSmartyVar) {
            ...
            $xoTheme->addMeta('meta', $property, htmlspecialchars($content, ENT_QUOTES, null, false));
        }
    }
    ...
    if ($useSmartyVar) {
        $xoopsTpl->assign('fbcomment_og_metas', $ourtags);
    }
...
function b_fbcomment_comment_show($options)
{
    ...
    if ($options[4]) {
        $ourcolor = ' data-colorscheme="dark"';
    } else {
        $ourcolor = '';
    }
    $block['colorscheme'] = $ourcolor;

    $intwidth = (int)$options[5];
    if ($intwidth < 1) {
        $ourwidth = '';
    } else {
        $ourwidth = ' data-width="' . $intwidth . '"';
    }
    $block['width'] = $ourwidth;

    $intposts = (int)$options[6];
    if ($intposts < 1) {
        $intposts = '10';
    }
...
{
    if (!$block = b_fbcomment_core_show($options, 'like')) {
        return false;
    }
    ...
    if ($options[6]) {
        $ourfaces = ' data-show-faces="true"';
    } else {
        $ourfaces = ' data-show-faces="false"';
    }
    $block['faces'] = $ourfaces;

    if ($options[7]) {
        $ouraction = ' data-action="recommend"';
    } else {
        $ouraction = '';
    }
    ...
    if (isset($options[9])) {
        $oursend = ' data-send="true"';
    } else {
        $oursend = ' data-send="false"';
    }
    $block['send'] = $oursend;
...
{
    if (!$block = b_fbcomment_core_show($options)) {
        return false;
    }
...
function b_fbcomment_activity_show($options)
{
    ...
    $block['appid'] = $moduleHelper->getConfig('facebook-appid');
    // channel for facebook js sdk, see: http://developers.facebook.com/blog/post/530/
    $block['channel'] = XOOPS_URL . '/modules/' . $dir . '/fbchannel.php?locale=' . _MB_FBCOM_SDK_CHANNEL_LOCALE;
    $block['locale']  = _MB_FBCOM_SDK_CHANNEL_LOCALE;
    ...
    if (!$options[4]) {
        $block['header'] = ' data-header="dark"';
    } else {
        $block['header'] = '';
    }

    if ($options[5]) {
        $block['colorscheme'] = ' data-colorscheme="dark"';
    } else {
        $block['colorscheme'] = '';
    }
    ...
    if ($options[7]) {
        $block['recommendations'] = ' data-recommendations="true"';
    } else {
...
function b_fbcomment_likebox_show($options)
{
    ...
    if ($options[4]) {
        $value = ' data-show-faces="true"';
    } else {
        $value = ' data-show-faces="false"';
    }
    $block['faces'] = $value;

    if ($options[5]) {
        $value = ' data-stream="true"';
    } else {
        $value = ' data-stream="false"';
    }
    $block['stream'] = $value;

    if ($options[6]) {
        $value = ' data-header="true"';
    } else {
        $value = ' data-header="false"';
    }
    $block['header'] = $value;
    ...
    if ($options[8]) {
        $value = ' data-force-wall="true"';
    } else {
        $value = ' data-force-wall="false"';
    }
...
function b_fbcomment_show_feed_show($options)
{
    ...
    $config           = array();
    $config['appId']  = $moduleHelper->getConfig('facebook-appid');
    $config['secret'] = $moduleHelper->getConfig('facebook-appsecret');
    ...
    */
    ...
    foreach ($data as $i => $item) {
        ...
        if (empty($data[$i]['permalink'])) {
            $data[$i]['permalink'] = $default_link;
        }
    }
    $block['feed'] = $data;

    $block['style'] = "<style>\n" . file_get_contents(XOOPS_ROOT_PATH . '/modules/fbcomment/assets/css/showfeed.css') . "\n</style>\n";

    //$xoTheme->addStyleSheet(XOOPS_URL.'/modules/fbcomment/assets/css/showfeed.css');
    return $block;
}
